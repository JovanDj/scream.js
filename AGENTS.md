...
* Keep templates presentation-only: escaped attribute references, presence checks, static includes with path-only parameters, and template application over pushed ViewModels, including static round-robin template lists. Applied templates read only `attr`; parameterized includes read only their named parameters. Template groups/skins are selected by `FileLoader`, not template syntax. Quote dynamic attribute values, use `HtmlAttributes` for prepared attribute lists, and do not add view-side loops, conditional-attribute directives, inline computation, hard-coded/composed URL attributes, or variables inside `<script>`/`<style>` bodies.
