...
* Keep templates presentation-only: escaped attribute references, presence checks, shallow interface declarations, static includes with explicit path-only parameters, and template application over pushed ViewModels, including static round-robin and chained application. Applied templates read only `attr` plus explicit parameters; includes never inherit parent context and read only explicitly supplied parameters. Template groups/skins are selected by `FileLoader`, not template syntax. Quote dynamic attribute values, use `HtmlAttributes` only for prepared attribute lists, and do not add view-side loops, conditional-attribute directives, inline computation, hard-coded/composed URL attributes, or variables inside `<script>`/`<style>` bodies.
* The template engine cannot prove pushed values are free of display/layout information; keep CSS classes, template names, URLs, prepared attributes, and formatted display strings out of domain models.
